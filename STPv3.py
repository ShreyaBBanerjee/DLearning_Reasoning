...
steps = 7500
batch = 10
recordstep = np.int64((steps/100) + 1)
...
# % of consistent problem
count_consistency = (np.count_nonzero(y_test == 1))/25000
baseaccur = np.full((recordstep), count_consistency)
itr = np.arange(0, 76, 1)
...
fig5 = plt.figure()
plt.plot(itr, baseaccur[itr], '-b', label='base accuracy')
plt.plot(itr, accur_test[itr], '-g', label='test accuracy')
plt.legend()
plt.xlabel('epoch')
plt.ylabel('accuracy')
plt.show()